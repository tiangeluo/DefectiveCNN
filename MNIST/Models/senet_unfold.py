...
class SENet(nn.Module):
    ...
    def forward(self, x):
        ...
        shortcut = tmp
        out = self.conv11(out)
        out = self.conv12(F.relu(self.bn12(out)))
        w = F.avg_pool2d(out, out.size(2))
        ...
        shortcut = tmp
        out = self.conv21(out)
        out = self.conv22(F.relu(self.bn22(out)))
        w = F.avg_pool2d(out, out.size(2))
        ...
        out = self.conv31(out)
        out = self.conv32(F.relu(self.bn32(out)))
        w = F.avg_pool2d(out, out.size(2))
        ...
        tmp = out
        out = F.relu(self.bn41(out))
        shortcut = tmp
        out = self.conv41(out)
        out = self.conv42(F.relu(self.bn42(out)))
        w = F.avg_pool2d(out, out.size(2))
